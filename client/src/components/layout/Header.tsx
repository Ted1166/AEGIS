...
export function Header({
  ...
  const isInitia = chainId === CHAIN_ID;
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
        {address ? (
          <>
            {!isInitia && <Badge variant="amber">Wrong Network</Badge>}
            {balance && isInitia && (
            ...
            >
              <span style={{
                ...
                background: isInitia ? 'var(--green)' : 'var(--amber)',
                animation: isInitia ? 'pulse-green 2s ease-in-out infinite' : 'none',
              }} />
              {short(address)}
            </button>
          </>
