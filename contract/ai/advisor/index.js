...
async function bootstrap() {
  ...
  console.log("╔══════════════════════════════════════════════╗");
  console.log("║      📋  Aegis Weekly Advisor                ║");
...
function _scheduleCron() {
  ...
  console.log(
    `[Advisor] Scheduled: day ${targetDay} (0=Sun) at ${targetHour}:00 UTC`
  );
...
async function _discoverUsers() {
  try {
    ...
    const events        = await vault.queryFilter(depositFilter);
  ...
  const emergencyEntries = await Promise.all(
    emergencies.map(async (e) => {
      ...
      return {
        ...
        name:      protocolName,
        timestamp: new Date(Number(e.args[2] || 0) * 1000).toISOString().split("T")[0]
                   || "this week",
...
function _startApiServer() {
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      if (wantHistory) {
        ...
        if (!latest) {
          res.statusCode = 404;
          return res.end(JSON.stringify({ error: "No report found for this address" }));
        }
  ...
  server.listen(CONFIG.apiPort, () => {
    ...
    console.log(`[Advisor]    GET /report/:address`);
    console.log(`[Advisor]    GET /report/:address/history`);
    console.log(`[Advisor]    GET /health`);
  });
}
...
function _describeRebalances(count, emergencies) {
  ...
  if (count > emergencies.length) {
    reasons.push(
      `Routine AI rebalance — allocation updated based on current Yield Quality Scores`
    );
  }
