...
const ORACLE_ADMIN_ABI = [
  "function addProtocol(address protocol) external",
  "function trackedProtocols(uint256) external view returns (address)",
  "function activeProtocolCount() external view returns (uint256)",
];
...
async function bootstrap() {
  ...
  console.log("╔══════════════════════════════════════════════╗");
  console.log("║        🧠  Aegis Yield Brain — Scorer        ║");
...
async function _ensureOracleTracking(sources) {
  const adminOracle = new ethers.Contract(
    CONFIG.oracleAddress,
    [...ORACLE_ABI, ...ORACLE_ADMIN_ABI],
    signer
  );
  ...
  for (const source of sources) {
    try {
      ...
      if (!entry.active) {
        console.log(`[Scorer] Adding new source to oracle: ${_short(source)}`);
        const tx = await adminOracle.addProtocol(source, { gasLimit: 150_000 });
        await tx.wait();
      ...
      try {
        const tx = await adminOracle.addProtocol(source, { gasLimit: 150_000 });
        await tx.wait();
        console.log(`[Scorer] ✅ Auto-tracked new source: ${_short(source)}`);
      } catch (addErr) {
...
async function _writeAllScores(results) {
  for (const result of results) {
    if (result.error) {
      console.warn(`[Scorer] Skipping ${_short(result.sourceAddress)} — score error: ${result.error}`);
      continue;
    }

    if (result.totalAPYBps === 0n) {
      console.warn(`[Scorer] Skipping ${_short(result.sourceAddress)} — zero APY reported`);
      continue;
    }
    ...
    try {
      ...
      totalErrors++;
      console.error(
        `[Scorer] Write failed for ${_short(result.sourceAddress)}: ${err.message}`
      );
    }
  }
}
